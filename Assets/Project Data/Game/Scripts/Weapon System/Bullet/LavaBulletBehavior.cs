...
namespace Watermelon.SquadShooter
{
    public class LavaBulletBehavior : PlayerBulletBehavior
    {
        ...
        public void Initialise(DuoInt damage, float speed, BaseEnemyBehavior currentTarget, float autoDisableTime,
            bool autoDisableOnHit, float shootingRadius, CharacterBehaviour characterBehaviour, DuoFloat bulletHeight,
            float explosionRadius)
        {
            ...
            var distanceMultiplier = Mathf.InverseLerp(0, shootingRadius,
                Vector3.Distance(characterBehaviour.transform.position, targetPosition));
            var bulletFlyTime = 1 / speed;

            _damageValue = damage;
            _movementTween = transform
                .DOBezierMove(targetPosition,
                    Mathf.Lerp(bulletHeight.firstValue, bulletHeight.secondValue, distanceMultiplier), 0, bulletFlyTime)
                .OnComplete(delegate { OnEnemyHitted(null); });
        ...
        protected override void OnEnemyHitted(BaseEnemyBehavior target)
        {
            ...
            foreach (var col in hitColliders)
            {
                ...
                var explosionDamageMultiplier = 1.0f - Mathf.InverseLerp(0, _explosionRadius,
                    Vector3.Distance(transform.position, col.transform.position));
                enemy.TakeDamage(CharacterBehaviour.NoDamage ? 0 : _damageValue.Lerp(explosionDamageMultiplier),
                    transform.position, (transform.position - _prevPosition).normalized);
