...
//if(enemyBehavior is MeleeEnemyBehaviour {isInStealth: true})return;
            if (enemyBehavior)
            {
                ...
                _ringTweenCase = _targetRing.transform.DOScale(Vector3.one * enemyBehavior.Stats.TargetRingSize, 0.2f).SetEasing(Ease.Type.BackIn);
