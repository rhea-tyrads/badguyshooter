...
namespace Watermelon
{
    ...
    public class ProjectInitSettingsEditor : Editor
    {
        ...
        readonly string CORE_MODULES_PROPERTY_NAME = "coreModules";
        readonly string MODULES_PROPERTY_NAME = "modules";
        readonly string MODULE_NAME_PROPERTY_NAME = "moduleName";
        static string DEFAULT_PROJECT_INIT_SETTINGS_PATH =
            "Assets/Project Data/Content/Settings/Project Init Settings.asset";
        SerializedProperty coreModulesProperty;
        SerializedProperty modulesProperty;
        List<InitModuleContainer> initModulesEditors;
        GUIContent addButton;
        GUIContent arrowDownContent;
        GUIContent arrowUpContent;
        GUIStyle arrowButtonStyle;
        ProjectInitSettings projectInitSettings;
        IEnumerable<Type> registeredTypes;
        GenericMenu modulesGenericMenu;
        [MenuItem("Tools/Editor/Project Init Settings")]
        public static void SelectProjectInitSettings()
        {
            var selectedObject =
                AssetDatabase.LoadAssetAtPath(DEFAULT_PROJECT_INIT_SETTINGS_PATH, typeof(ProjectInitSettings));
            ...
            if (selectedObject == null)
            {
                ...
                else
                {
                    ...
                    Debug.LogWarning(
                        $"Asset with type \"ProjectInitSettings\" is misplaced. Expected path: {DEFAULT_PROJECT_INIT_SETTINGS_PATH} .Actual path: {AssetDatabase.GetAssetPath(selectedObject)}");
                }
            }
            else
            {
        ...
        protected void OnEnable()
        {
            ...
            registeredTypes = Assembly.GetAssembly(typeof(InitModule)).GetTypes().Where(type =>
                type.IsClass && !type.IsAbstract && type.IsSubclassOf(typeof(InitModule)));
            InitGenericMenu();
            InitCoreModules();
            LoadEditorsList();
        ...
        void InitCoreModules()
        {
            ...
            var coreModules = projectInitSettings.CoreModules;

            var newModuleCreated = false;
            foreach (var type in registeredTypes)
            {
                var defineAttributes =
                    (RegisterModuleAttribute[]) Attribute.GetCustomAttributes(type, typeof(RegisterModuleAttribute));
                foreach (var module in defineAttributes)
                {
                    if (!module.Core) continue;
                    var isExists = coreModules != null && coreModules.Any(x => x.GetType() == type);
                    if (isExists) continue;
                    if (!EditorUtility.DisplayDialog("New core module is found!",
                            $"Module {type.FullName} is required for propper work of the game." +
                            $" Do you want to add it to modules list?", "Add", "Skip")) continue;
                    AddCoreModule(type);
        ...
        void InitGenericMenu()
        {
            ...
            //Load all modules
            var initModules = projectInitSettings.Modules;
            ...
            foreach (var type in registeredTypes)
            {
                ...
                {
                    if (module.Core) continue;
                    var isAlreadyActive = initModules != null && initModules.Any(x => x.GetType() == type);
                    if (isAlreadyActive)
                        modulesGenericMenu.AddDisabledItem(new GUIContent(module.Path), false);
                    else
                    {
                        modulesGenericMenu.AddItem(new GUIContent(module.Path), false, delegate
                        {
                            AddModule(type);
                            InitGenericMenu();
                        });
        ...
        void LogPlayModeState(PlayModeStateChange obj)
        {
            if (Selection.activeObject == target)
                Selection.activeObject = null;
        }

        void LoadEditorsList()
        {
            ClearInitModules();
            SerializedProperty initModule;
            SerializedObject initModuleSerializedObject;

            for (var i = 0; i < coreModulesProperty.arraySize; i++)
            {
                initModule = coreModulesProperty.GetArrayElementAtIndex(i);
                if (initModule.objectReferenceValue == null) continue;
                initModuleSerializedObject = new SerializedObject(initModule.objectReferenceValue);
                initModulesEditors.Add(new InitModuleContainer(initModule.objectReferenceValue.GetType(),
                    initModuleSerializedObject, Editor.CreateEditor(initModuleSerializedObject.targetObject)));
            }
        ...
        InitModuleContainer GetEditor(Type type) => initModulesEditors.FirstOrDefault(module => module.type == type);
        ...
        void ClearInitModules()
        {
            if (initModulesEditors != null)
            {
                // Destroy old editors
                foreach (var module in initModulesEditors.Where(module => module != null && module.editor != null))
                {
                    DestroyImmediate(module.editor);
                }
        ...
        void DrawInitModule(SerializedProperty arrayProperty, string title, bool buttons)
        {
            ...
            if (arrayProperty.arraySize > 0)
            {
                ...
                {
                    var initModule = arrayProperty.GetArrayElementAtIndex(i);
                    ...
                    if (initModule.objectReferenceValue != null)
                    {
                        ...
                        EditorGUILayout.LabelField(moduleNameProperty.stringValue,
                            WatermelonEditor.Styles.label_medium);
                        EditorGUILayout.EndHorizontal();
                        if (initModule.isExpanded)
                        {
                            var moduleContainer = GetEditor(initModule.objectReferenceValue.GetType());
                            ...
                            if (buttons)
                            {
                                ...
                                {
                                    if (EditorUtility.DisplayDialog("This object will be removed!", "Are you sure?",
                                            "Remove", "Cancel"))
                                    {
                        ...
                        if (GUI.Button(new Rect(moduleRect.x + moduleRect.width - 15, moduleRect.y, 12, 12),
                                arrowUpContent, arrowButtonStyle))
                        {
                            if (i > 0)
                            {
                                var expandState = arrayProperty.GetArrayElementAtIndex(i - 1).isExpanded;
                    ...
                    else
                    {
                        ...
                        EditorGUILayout.LabelField(EditorGUIUtility.IconContent("console.warnicon"),
                            WatermelonEditor.Styles.padding00, GUILayout.Width(16), GUILayout.Height(16));
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.EndVertical();
            GUILayout.Space(5);
            var projectModulesRect = EditorGUILayout.BeginVertical(WatermelonEditor.Styles.Skin.box);
        ...
        public void AddModule(Type moduleType)
        {
            ...
            var testInitModule = (InitModule) ScriptableObject.CreateInstance(moduleType);
        ...
        public void AddCoreModule(Type moduleType, bool updateFileSystem = false)
        {
            ...
            testInitModule.name = moduleType.ToString();
            AssetDatabase.AddObjectToAsset(testInitModule, target);
            coreModulesProperty.GetArrayElementAtIndex(coreModulesProperty.arraySize - 1).objectReferenceValue =
                testInitModule;
            serializedObject.ApplyModifiedProperties();

            if (!updateFileSystem) return;
            LoadEditorsList();
            EditorUtility.SetDirty(target);
            AssetDatabase.SaveAssets();
        }

        class InitModuleContainer
        {
            public Type type;
